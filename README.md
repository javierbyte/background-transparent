...
2. **Find the viewport.** Two small color markers (fiducials) are injected at the top corners of the viewport. On the first frame the code picks the two candidate colors least present on screen to avoid false positives.
   ...
   - An interesting fact is that I tried to use `window.screenX` and `window.screenY`, and while it works, there are many issues: these values seem to be updated only 15 times per second in both Chrome and Safari, vs 30fps we get from the video feed. Also, cutting the hole to update the buffer (step 4) is harder when the screen position and the video frame to cut are not coming at the exact same time.
...
I ended up separating the "measure and update buffer" step from a different one that translates the visible viewport so I can add interpolation to the viewport translation.
