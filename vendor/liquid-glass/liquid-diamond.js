// Vanilla JS Liquid Diamond (Pyramid) Effect
// Created by Shu Ding (https://github.com/shuding/liquid-glass) in 2025.
// Modified: removed auto-init, removed drag/physics, exposed factory with show/hide.
...
;(function () {
  ...
  function attachLiquidPyramid(options) {
    ...
    var THETA_PER_PX = 0.0125
    var PHI_PER_PX = 0.0125
    var PHI_MIN = -1.35
    var PHI_MAX = 1.35

    function renderShader(theta, phi) {
      var context = createPyramidContext(theta, phi)

      var centerX = (context.hullCenter.x / WIDTH) * 100
      var centerY = (context.hullCenter.y / HEIGHT) * 100
      var topX = (context.bloomAnchors.top.x / WIDTH) * 100
      var topY = (context.bloomAnchors.top.y / HEIGHT) * 100
      var leftX = (context.bloomAnchors.lowerLeft.x / WIDTH) * 100
      var leftY = (context.bloomAnchors.lowerLeft.y / HEIGHT) * 100
      var rightX = (context.bloomAnchors.lowerRight.x / WIDTH) * 100
      var rightY = (context.bloomAnchors.lowerRight.y / HEIGHT) * 100
      var bloomScaleX = Math.max((context.hullBounds.width / WIDTH) * 100, 68)
      var bloomScaleY = Math.max((context.hullBounds.height / HEIGHT) * 100, 78)
      ...
      shader.setFragment(function (uv) {
        var trace = tracePyramidRay(uv, context)
        if (!trace) {
          return texture(uv.x, uv.y)
        }
        var bulgedTrace = applyEdgeBulge(trace, uv, context)
        return texture(bulgedTrace.displacedUv.x, bulgedTrace.displacedUv.y)
      })
    }

    // Fixed centered, no position offset
    host.style.transform = 'translate(-50%, -50%)'

    // Initial render at rest rotation
    renderShader(0.64, -0.42)
    ...
    return {
      ...
      setRotation: function (ox, oy) {
        var theta = 0.64 + ox * THETA_PER_PX
        var phi = clamp(-0.42 - oy * PHI_PER_PX, PHI_MIN, PHI_MAX)
        renderShader(theta, phi)
      },
      destroy: function () {
        shader.destroy()
        mount.remove()
